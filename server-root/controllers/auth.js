...
const { authCookieName } = require('../server-config');
    ...
    return userModel.create({ tel, email, username, password })
        .then((createdUser) => {
            ...
            if (process.env.NODE_ENV === 'production') {
                ...
                res.cookie(authCookieName, token, { httpOnly: true, secure: true })
            }
            res.status(200)
                .send(createdUser);
        })
...
function login(req, res, next) {
    ...
    userModel.findOne({ email })
        ...
        .then(([user, match]) => {
            ...
            res.cookie(authCookieName, token, maxCookieAge[rememberMe]);
            res.status(200)
                .send(user);
        })
        .catch(next);
